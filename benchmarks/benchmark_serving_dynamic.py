...
async def benchmark(
    ...
    test_output = await request_func(policy='fcfs', request_func_input=test_input)
    ...
    if profile:
        ...
        profile_output = await request_func(policy='fcfs', request_func_input=profile_input)
    ...
    async for request in get_request(input_requests, request_rate, request_rates, rate_change_interval):
        ...
        tasks.append(
            asyncio.create_task(
                request_func(policy='fcfs', request_func_input=request_func_input,
                             pbar=pbar)))
...
def main(args: argparse.Namespace):
    ...
    if len(args.request_rates) > 0:
        request_rates = args.request_rates.split(",")
        request_rates = [float(rate) for rate in request_rates] 
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Comma-separated list of request rates to benchmark. "
        "If not set, then the request rate is fixed at "
        "`request_rate`.",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=10,
        help="Interval between requests. Only used when `request-rates` "
        "is not none.",
    )
